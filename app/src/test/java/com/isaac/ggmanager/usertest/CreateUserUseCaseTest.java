...
public class CreateUserUseCaseTest {
    ...
    @Test
    public void execute_shouldReturnSuccess_whenUserCreated() {
        // Arrange
        UserModel user = new UserModel();
        user.setFirebaseUid("firebaseUid123");
        user.setEmail("test@example.com");

        MutableLiveData<Resource<Boolean>> liveData = new MutableLiveData<>();
        liveData.setValue(Resource.success(true));

        when(userRepository.create(user)).thenReturn(liveData);

        // Act
        LiveData<Resource<Boolean>> result = createUserUseCase.execute(user);
